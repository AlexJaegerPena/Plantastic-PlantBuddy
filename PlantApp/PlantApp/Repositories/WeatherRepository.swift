...
class WeatherRepository {
    ...
    func fetchCurrentWeather(for city: String? = nil, coordinates: CLLocationCoordinate2D? = nil) async throws -> WeatherResponse {
        ...
        if let city {
            ...
            urlComponents.queryItems?.append(
                URLQueryItem(
                    name: "q", value: "\(coords.latitude),\(coords.longitude)"))
        } else {
            // Wenn weder Stadt noch Koordinaten angegeben sind
            throw WeatherAPIError.apiError(
                "Keine Stadt oder Koordinaten für die Wetterabfrage angegeben.")
        }
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse,
                ...
                let errorData =
                    ...
                    as? [String: Any]
                let errorMessage =
                    (errorData?["error"] as? [String: Any])?["message"]
                    as? String ?? "Unbekannter API-Fehler"
