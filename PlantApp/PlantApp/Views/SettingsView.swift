...
struct SettingsView: View {
    ...
    var getTimeFromDate: [String] {
        date.formatted(date: .omitted, time: .shortened).components(
            separatedBy: ":")
    }
                ...
                if editing {
                    ...
                    VStack {
                        if showPassword {
                            ...
                            SecureField(
                                "Password", text: $userViewModel.password)
                        }
                    }
                    .overlay {
                        HStack {
            ...
            Section("App settings") {
                ...
                VStack(alignment: .leading) {
                    Toggle(
                        ...
                        isOn: $notificationsViewModel.allowNotifications
                    )
                    ...
                    if notificationsViewModel.allowNotifications {
                        VStack {
                            DatePicker(
                                "Set Time", selection: $date,
                                displayedComponents: .hourAndMinute)

                            Button {
                                notificationsViewModel.wateringNotification(
                                    hour: Int(getTimeFromDate[0]) ?? 0,
                                    minute: Int(getTimeFromDate[1]) ?? 0)
